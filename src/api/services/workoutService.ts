...
export interface Guide {
  ...
  title: string
  image_url: string
  guide_type: number // 0 = Guide, 1 = Training
  created_at: string
  updated_at: string
  weeks?: Week[]
}
...
export const workoutService = {
  ...
  async getWeek(id: string) {
    const { data: week, error: weekError } = await supabase.from("weeks").select("*").eq("id", id).single()
  ...
  async getSession(id: string) {
    ...
    const exerciseOrderMap = new Map(sessionExercises.map((se) => [se.exercise_id, se.order_in_session]))
    ...
    const { data: allSets, error: allSetsError } = await supabase.from("sets").select("*").in("id", setIds)
  ...
  async getExercise(id: string) {
    const { data: exercise, error: exerciseError } = await supabase.from("exercises").select("*").eq("id", id).single()

    if (exerciseError) throw exerciseError

    const { data: sets, error: setsError } = await supabase.from("exercise_sets").select("*").eq("exercise_id", id)
